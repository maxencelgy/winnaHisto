#!/usr/bin/env python3
"""Combos legs=2 mid-cote (1.30-1.50) - cote totale ~1.8-2.25 par palier.
Plus WR≥0.90-0.95 ultra-strict sur micro-cote.
"""
import sys, os, json
...
# B. Ultra-strict WR (0.90, 0.92, 0.95) sur micro-cote
for sports, sname in [(["football","ice-hockey"],"FH"),
                       (["football","ice-hockey","tennis"],"FHT"),
                       (["football","ice-hockey","tennis","basketball"],"FHTB")]:
    for cmin, cmax in [(1.05,1.12), (1.05,1.15), (1.08,1.18), (1.10,1.20)]:
        for mwr in [0.88, 0.90, 0.92, 0.95]:
            for n_p in [3, 4, 5]:
                for sort in ["wr", "ev"]:
                    CANDS.append({
                        "id": f"USTR_{sname}_{cmin}-{cmax}_wr{int(mwr*100)}_p{n_p}_{sort}",
                        "components": [{"sports": sports, "market": "1x2,btts,over_1_5,over_2_5",
                                        "cote_min": cmin, "cote_max": cmax,
                                        "sort_by": sort, "max_legs": 1, "max_combos": 1,
                                        "min_wr": mwr, "min_ev": None,
...
print(f"[l2m_ustr] {len(CANDS)} configs")
...
for i, s in enumerate(CANDS):
    ...
    try:
        ...
        if r["n_cycles_total"] >= 25 and r["final_pnl"] > 800:
            results.append({
                "id": s["id"],
...
with open("/tmp/find_legs2_midcote_ultra.json","w") as f: json.dump(results,f,indent=2)
print("\nSaved")
