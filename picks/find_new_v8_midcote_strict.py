#!/usr/bin/env python3
"""Mid-cote 1.40-1.65 + WR strict ≥75% — zone non explorée entre 1.20-1.40 et 1.65+."""
import sys, os, json
...
# A. Foot xmkt mid-cote WR strict
for cmin, cmax in [(1.35, 1.55), (1.40, 1.60), (1.45, 1.65), (1.40, 1.55)]:
    for mkt in ["btts,over_1_5,over_2_5", "over_1_5,over_2_5", "1x2,over_1_5,over_2_5"]:
        for mc in [3, 5, 8]:
            for mwr in [0.65, 0.70, 0.75]:
                for pct in [0.05, 0.07, 0.10, 0.15]:
                    CANDS.append({
                        "id": f"MID_F_{mkt[:5]}_{cmin}-{cmax}_mc{mc}_wr{mwr}_pct{int(pct*100)}",
...
# B. Multi-comp F+H mid-cote
for cmin, cmax in [(1.35, 1.55), (1.40, 1.60)]:
    for foot_wr in [0.65, 0.70]:
        for hockey_wr in [0.65, 0.70]:
            for f_mc, h_mc in [(3, 3), (3, 5), (5, 5)]:
                for pct in [0.05, 0.07, 0.10]:
                    CANDS.append({
                        "id": f"MID_FH_{cmin}-{cmax}_fw{foot_wr}_hw{hockey_wr}_F{f_mc}H{h_mc}_pct{int(pct*100)}",
                        "components": [
...
print(f"[Mid-cote] {len(CANDS)} configs")
...
with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/new_v8_midcote.json","w") as f:
