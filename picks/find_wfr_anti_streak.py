#!/usr/bin/env python3
"""Sweep anti-streak — sizing qui s'adapte selon les loss récents.
+ Multi-comp avec ajout tennis ou foot mid-cote 1.40-1.55."""
import sys, os, json
...
# A. Multi-comp avec tennis (jamais testé en WFR)
for foot_mc in [3, 5]:
    for hockey_mc in [3, 5]:
        for tennis_mc in [1, 2, 3]:
            for pct in [0.05, 0.07, 0.10]:
                CANDS.append({
                    "id": f"AS_FT_F{foot_mc}H{hockey_mc}T{tennis_mc}_pct{int(pct*100)}",
                    "components": [
...
# B. Foot xmkt cote 1.40-1.55 (entre 1.20-1.40 et 1.45-1.65 testées)
for foot_mc in [3, 5, 7]:
    for hockey_mc in [3, 5]:
        for pct in [0.05, 0.07, 0.10]:
            for cote_range in [(1.35, 1.55), (1.40, 1.60)]:
                CANDS.append({
                    "id": f"AS_F{foot_mc}H{hockey_mc}_{cote_range[0]}-{cote_range[1]}_pct{int(pct*100)}",
                    "components": [
                        {"sport": "football", "market": "btts,over_1_5,over_2_5",
...
# C. Multi-comp avec foot OU + Hockey + Basket en cote moyenne
for foot_mc in [3, 5]:
    for hockey_mc in [3, 5]:
        for basket_mc in [2, 3]:
            for pct in [0.05, 0.07]:
                CANDS.append({
                    "id": f"AS_3S_F{foot_mc}H{hockey_mc}B{basket_mc}_pct{int(pct*100)}",
                    "components": [
                        {"sport": "football", "market": "btts,over_1_5,over_2_5",
...
print(f"[Anti-streak/new angles] {len(CANDS)} configs")
...
with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/wfr_anti_streak.json","w") as f:
