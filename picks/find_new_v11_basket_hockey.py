#!/usr/bin/env python3
"""Basket+Hockey duo isolé (sans foot) — angle non testé en isolation."""
import sys, os, json
...
# Basket+Hockey duo strict
for cmin, cmax in [(1.20, 1.40), (1.20, 1.50), (1.30, 1.55), (1.20, 1.45)]:
    for basket_wr in [0.65, 0.70, 0.75]:
        for hockey_wr in [0.65, 0.70, 0.75, 0.80]:
            for b_mc in [1, 2, 3, 5]:
                for h_mc in [3, 5, 8]:
                    for pct in [0.05, 0.07, 0.10, 0.15]:
                        CANDS.append({
                            "id": f"BH_{cmin}-{cmax}_bw{basket_wr}_hw{hockey_wr}_B{b_mc}H{h_mc}_pct{int(pct*100)}",
                            "components": [
...
print(f"[Basket+Hockey] {len(CANDS)} configs")
...
print(f"\n=== TOP 10 par DD le plus bas (record 22€, pnl≥200, BR≥2) ===")
for r in ddok[:10]:
    flag = " 🏆" if r["dd"] < 22 else ""
    print(f"  DD {r['dd']:>4.0f}€ ratio {r['ratio']:>5.1f} BR×{r['br_mult']:>5.1f} | +{r['pnl']:>5.0f}€ #{r['n_combos']:>3d} | {r['id'][:60]}{flag}")

with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/new_v11_bh.json","w") as f:
