#!/usr/bin/env python3
"""Combos legs=6 et legs=7 mid-cote — extension du record legs=5 +23378€."""
import sys, os, json
...
for sports, sname in [(["football","ice-hockey"],"FH"),
                       ...
                       (["football"],"F")]:
    for cmin, cmax in [(1.20,1.30), (1.20,1.35), (1.25,1.35), (1.25,1.40), (1.30,1.40)]:
        for mwr in [None, 0.65, 0.70, 0.75]:
            for legs in [6, 7]:
                for n_p in [1, 2, 3]:
                    for sort in ["wr", "ev"]:
                        CANDS.append({
                            "id": f"L{legs}_{sname}_{cmin}-{cmax}_wr{mwr}_p{n_p}_{sort}",
...
# Mode INTERDAY combos
for sports, sname in [(["football","ice-hockey","tennis"],"FHT"),
                       (["football","ice-hockey"],"FH")]:
    for cmin, cmax in [(1.20,1.35), (1.25,1.40)]:
        for mwr in [0.70, 0.75]:
            for legs in [3, 4, 5]:
                for n_p in [3, 5]:
                    for sort in ["wr", "ev"]:
                        CANDS.append({
                            "id": f"ID_L{legs}_{sname}_{cmin}-{cmax}_wr{int(mwr*100)}_p{n_p}_{sort}_INTER",
                            "components": [{"sports": sports, "market": "1x2,btts,over_1_5,over_2_5",
                                            "cote_min": cmin, "cote_max": cmax,
                                            "sort_by": sort, "max_legs": 1, "max_combos": 1,
                                            "min_wr": mwr, "min_ev": None,
...
print(f"[l67] {len(CANDS)} configs")

results = []
for i, s in enumerate(CANDS):
    if i % 100 == 0: print(f"  [{i}/{len(CANDS)}]")
    try:
        mode = s.pop("_mode", "intraday") if "_mode" in s else "intraday"
        r = simulate(s, START, END, mode=mode, initial_stake=INITIAL)
        if r["n_cycles_total"] >= 20 and r["final_pnl"] > 5000:
            results.append({
                "id": s["id"],
...
with open("/tmp/find_legs67_extreme.json","w") as f: json.dump(results,f,indent=2)
print("\nSaved")
