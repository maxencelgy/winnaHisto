#!/usr/bin/env python3
"""Sweep montantes avec min_wr strict — voir si filtre WR pousse l'EV pratique."""
import sys, os, json
        ...
        for legs in [1, 2]:
            for n_p in [3, 4, 5, 6]:
                for mwr in [0.70, 0.75, 0.80]:
                    CANDS.append({
                        "id": f"MWS_{mkt.replace(',','+')}_{cmin}-{cmax}_l{legs}_p{n_p}_wr{mwr}",
                        "label": "Montante WR strict",
                        "components": [{
...
print(f"[Montantes WR strict] {len(CANDS)} configs")
...
with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/mws.json","w") as f:
