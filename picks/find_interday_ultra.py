#!/usr/bin/env python3
"""INTERDAY ULTRA : 14-15 paliers + combos legs=4-5 — pousser au-delà de +640k€."""
import sys, os, json
...
for sports, sname in [(["football","ice-hockey","tennis"],"FHT"),
    ...
    for cmin, cmax in [(1.18,1.32), (1.20,1.32), (1.20,1.35)]:
        for mwr in [0.65, 0.70, 0.75]:
            for legs in [4, 5]:
                for n_p in [14, 15, 18, 20]:
                    for sort in ["wr", "ev"]:
                        CANDS.append({
                            "id": f"IU_{sname}_{cmin}-{cmax}_wr{int(mwr*100)}_l{legs}_p{n_p}_{sort}",
...
for i, s in enumerate(CANDS):
    ...
    try:
        ...
        if r["n_cycles_total"] >= 5 and r["final_pnl"] > 50000:
            results.append({
                "id": s["id"],
...
print(f"\n[iu] {len(results)} viable (PnL>50k)")
results.sort(key=lambda r: -r["pnl"])
print(f"\n=== TOP 15 par PnL ===")
for r in results[:15]:
    print(f"  {r['comp']*100:>5.2f}% {r['n_comp']:>3}/{r['n_tot']:<3} cap{r['cap']:>7.0f} pnl{r['pnl']:>+8.0f}  {r['id']}")

with open("/tmp/find_interday_ultra.json","w") as f: json.dump(results,f,indent=2)
print("\nSaved")
