#!/usr/bin/env python3
"""Sweep progressives v2 — MASSIF (60+ configs).
Test toutes combinaisons cote/sport/legs pour valider best progressive."""
...
def simulate_progressive(palier_configs, start_date, end_date, mode="intraday", initial_stake=10):
    n_target = len(palier_configs)
    ...
    def reset_cycle(reason):
        ...
        cycles_log.append({"outcome": reason, "n_paliers_won": palier,
    ...
    for d in days:
        ...
        while palier < n_target and not day_done:
            ...
            if won_all:
                ...
                if palier >= n_target:
                    reset_cycle("complete"); break
            else:
                reset_cycle("loss"); break
    ...
    return {
        "n_complete": n_complete, "n_total": n_total_cycles,
...
def cfg(sports, market, cmin, cmax, legs=1, sort="wr"):
    return {"sports": sports, "market": market, "cote_min": cmin, "cote_max": cmax,
            "legs_per_palier": legs, "sort_by": sort}


# === MEGA configs progressives ===
CONFIGS = []

# A. Hockey progressive cote up (2-4 paliers)
for n_p in [2, 3, 4]:
    for start_cmin in [(1.10, 1.25), (1.15, 1.30), (1.20, 1.35)]:
        end_cmin = (1.40, 1.65) if n_p >= 3 else (1.30, 1.50)
        if n_p == 2:
            paliers = [
                cfg(["ice-hockey"], "1x2", *start_cmin),
                cfg(["ice-hockey"], "1x2", *end_cmin),
            ]
        elif n_p == 3:
            paliers = [
                cfg(["ice-hockey"], "1x2", *start_cmin),
                cfg(["ice-hockey"], "1x2", 1.25, 1.45),
                cfg(["ice-hockey"], "1x2", *end_cmin),
            ]
        else:
            paliers = [
        ...
        CONFIGS.append((f"Hockey_progressive_{start_cmin[0]}-{end_cmin[1]}_p{n_p}", paliers))

# B. Foot O 1.5 progressive cote up (2-4 paliers)
for n_p in [2, 3, 4, 5]:
    if n_p == 2:
        paliers = [
        ...
        paliers = [
            ...
            cfg(["football"], "over_1_5", 1.35, 1.55),
            cfg(["football"], "over_1_5", 1.50, 1.75),
        ]
    else:  # 5
        paliers = [
            cfg(["football"], "over_1_5", 1.10, 1.20),
            cfg(["football"], "over_1_5", 1.15, 1.30),
            cfg(["football"], "over_1_5", 1.25, 1.45),
            cfg(["football"], "over_1_5", 1.40, 1.60),
            cfg(["football"], "over_1_5", 1.55, 1.80),
        ]
    CONFIGS.append((f"Foo_O15_progressive_p{n_p}", paliers))

# C. Aggressive then safer (Hockey)
for n_p in [3, 4]:
    if n_p == 3:
        paliers = [
        ...
        paliers = [
            cfg(["ice-hockey"], "1x2", 1.50, 1.80),
            cfg(["ice-hockey"], "1x2", 1.40, 1.60),
            cfg(["ice-hockey"], "1x2", 1.30, 1.50),
            cfg(["ice-hockey"], "1x2", 1.20, 1.40),
        ]
    CONFIGS.append((f"Hockey_aggro_then_safer_p{n_p}", paliers))

# D. Multi-sport pool progressive (élargit le pool à chaque palier)
for n_p in [3, 4]:
    if n_p == 3:
        paliers = [
            cfg(["football"], "over_1_5", 1.20, 1.40),
            cfg(["football", "ice-hockey"], "1x2", 1.30, 1.50),
            cfg(["football", "ice-hockey", "basketball"], "1x2", 1.40, 1.65),
        ]
    else:
        paliers = [
            cfg(["football"], "over_1_5", 1.20, 1.35),
            cfg(["football", "ice-hockey"], "1x2", 1.25, 1.45),
            cfg(["football", "ice-hockey", "basketball"], "1x2", 1.35, 1.55),
            cfg(["football", "ice-hockey", "basketball"], "1x2", 1.45, 1.70),
        ]
    CONFIGS.append((f"Multi_sport_pool_progressive_p{n_p}", paliers))

# E. Combo progression (single → combo 2j)
for n_p in [2, 3]:
    if n_p == 2:
        paliers = [
        ...
        paliers = [
            cfg(["football"], "over_1_5", 1.20, 1.35, legs=1),
            cfg(["football"], "over_1_5", 1.15, 1.30, legs=2),
            cfg(["football"], "over_1_5", 1.10, 1.22, legs=3),
        ]
    CONFIGS.append((f"Combo_progression_legs_p{n_p}", paliers))

# F. Sport rotation (différent sport chaque palier - test contrôlé)
for n_p in [2, 3]:
    if n_p == 2:
        paliers = [
            cfg(["football"], "over_1_5", 1.20, 1.40),
        ...
        paliers = [
            cfg(["football"], "over_1_5", 1.20, 1.40),
            cfg(["ice-hockey"], "1x2", 1.30, 1.50),
            cfg(["basketball"], "1x2", 1.30, 1.50),
        ]
    CONFIGS.append((f"Sport_rotation_p{n_p}", paliers))

# G. Cote constante mais sort by EV par palier
for cmin, cmax in [(1.30, 1.55), (1.40, 1.65)]:
    for n_p in [2, 3]:
        paliers = []
        for _ in range(n_p):
            paliers.append(cfg(["ice-hockey"], "1x2", cmin, cmax, sort="ev"))
        CONFIGS.append((f"Hockey_const_ev_{cmin}-{cmax}_p{n_p}", paliers))

# H. Foot O 1.5 mid-cote, paliers identiques (référence)
for cmin, cmax in [(1.30, 1.50), (1.35, 1.55)]:
    for n_p in [3, 4]:
        paliers = [cfg(["football"], "over_1_5", cmin, cmax)] * n_p
        CONFIGS.append((f"Foot_O15_const_{cmin}-{cmax}_p{n_p}", paliers))

# I. Hockey + xmkt foot mix
paliers = [
    cfg(["ice-hockey"], "1x2", 1.20, 1.40),
    cfg(["football"], "1x2,btts,over_1_5,over_2_5", 1.30, 1.50, legs=2),
]
CONFIGS.append(("Hockey_then_foot_xmkt_combo_p2", paliers))

print(f"=== {len(CONFIGS)} configs progressives à tester (Winamax FR strict) ===\n")
print(f"{'Config':<55s} {'#✓/tot':>10s} {'%':>5s} {'Cap':>7s} {'PnL':>9s}")
print("-"*100)
results = []
for name, paliers in CONFIGS:
    try:
        r = simulate_progressive(paliers, "2026-01-01", "2026-04-30", mode="intraday", initial_stake=10)
        results.append({"name": name, "config": paliers, **r})
        print(f"{name[:54]:<55s} {r['n_complete']:>3d}/{r['n_total']:<3d} {r['completion_rate']*100:>4.0f}% {r['avg_capital_complete']:>6.0f}€ +{r['final_pnl']:>5.0f}€")
...
print(f"\n=== TOP 15 par PnL ===")
for r in results[:15]:
    ev = r["final_pnl"] * r["completion_rate"]
    print(f"  EV {ev:>5.0f}  {r['name'][:55]:<55s} {r['n_complete']:>2d}/{r['n_total']:<3d} ({r['completion_rate']*100:.0f}%) cap{r['avg_capital_complete']:>4.0f}€ +{r['final_pnl']:.0f}€")

with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/progressive_v2.json","w") as f:
