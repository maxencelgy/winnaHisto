#!/usr/bin/env python3
"""Sweep multi-comp aggro WFR — pousser BR mult avec multi-sport diversifié."""
import sys, os, json
...
# Multi-comp: variations exhaustives Foot OU + Hockey + Basket + sizing
for foot_mc in [3, 5, 7, 10]:
    for hockey_mc in [2, 3, 5, 7]:
        for basket_mc in [0, 1, 2, 3]:
            for pct in [0.05, 0.07, 0.08, 0.10]:
                comps = [{
                ...
                CANDS.append({
                    "id": f"MCA_F{foot_mc}H{hockey_mc}B{basket_mc}_pct{int(pct*100)}",
                    "components": comps,
                    "dedup": "max1",
                    "sizing": {"mode": "flat_pct", "pct": pct, "min_stake": 0.5},
                })

# Multi-comp avec foot xmkt
for foot_mc in [3, 5, 7]:
    for hockey_mc in [2, 3, 5]:
        for pct in [0.05, 0.07, 0.10]:
            CANDS.append({
                "id": f"MCA_xmkt_F{foot_mc}H{hockey_mc}_pct{int(pct*100)}",
                "components": [{
                    "sport": "football", "market": "over_1_5,over_2_5",
...
print(f"[Multi-comp aggro] {len(CANDS)} configs WFR")
...
# Filter ratio ≥5 + BR ≥10
viable = [r for r in results if r["ratio"] >= 5 and r["br_mult"] >= 10]
viable.sort(key=lambda r: -r["br_mult"])

print(f"\n[Multi-comp] {len(viable)} viables (ratio ≥5, BR ≥10)")

print(f"\n=== TOP 20 par BR multiplier ===")
for r in viable[:20]:
    print(f"  BR×{r['br_mult']:>5.1f} ratio {r['ratio']:>4.1f}× | +{r['pnl']:>5.0f}€ DD {r['dd']:>4.0f}€ #{r['n_combos']:>4d}  | {r['id'][:50]}")

print(f"\n=== TOP 15 par RATIO (BR ≥10) ===")
viable.sort(key=lambda r: -r["ratio"])
for r in viable[:15]:
    print(f"  Ratio {r['ratio']:>5.1f}× BR×{r['br_mult']:>4.1f} | +{r['pnl']:>5.0f}€ DD {r['dd']:>4.0f}€ #{r['n_combos']:>4d} | {r['id'][:50]}")

with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/wfr_multicomp.json","w") as f:
