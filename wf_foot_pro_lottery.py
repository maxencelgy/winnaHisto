#!/usr/bin/env python3
"""Walk-forward Foot_pro_lottery sur 5 semestres sous filtre Winamax FR."""
...
print("=== WALK-FORWARD Foot_pro_lottery — Winamax FR + dédup max1 ===")
print(f"{'Semestre':10s} {'n':>5s} {'jours+/-':>10s} {'win%':>5s} {'PnL':>7s} {'ROI%':>7s} {'DD':>5s} {'série':>6s}")
print("-" * 75)
rois = []
for name, sd, ed in semesters:
    p = {"components":components, "date":sd, "end_date":ed, "sizing":"flat",
         "stake":"10", "bankroll":"100", "bookmaker":"winamax_fr", "dedup":"max1"}
    d = run(p)
    if d.get("error"): print(f"{name:10s} ERR: {d['error'][:50]}"); continue
    streak = 0; cur = 0
    for day in d["daily"]:
...
print()
print(f"ROI moyen: {sum(rois)/len(rois):+.1f}%")
print(f"ROI min: {min(rois):+.1f}% | max: {max(rois):+.1f}% | std: {(sum((r-sum(rois)/len(rois))**2 for r in rois)/len(rois))**0.5:.1f}pts")
print(f"Tous semestres > +30% : {'OUI ✓ VALIDÉ' if all(r > 30 for r in rois) else 'NON — overfit risque'}")
