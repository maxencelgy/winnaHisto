...
configs = [
    (2, 1.70, 1.78, "2p_173"),
    (3, 1.42, 1.48, "3p_145"),
    (3, 1.43, 1.46, "3p_145_strict"),
    (4, 1.30, 1.36, "4p_133"),
    (4, 1.31, 1.34, "4p_132_strict"),
    (5, 1.23, 1.28, "5p_125"),
    (6, 1.18, 1.23, "6p_120"),
]
for n_p, cmin, cmax, tag in configs:
    for sport, mkt in [("football", "over_1_5"), ("football", "over_1_5,over_2_5"),
        ...
        for mwr in [0.65, 0.70, 0.75, 0.80]:
            CANDS.append({
                "id": f"X3_{tag}_{sport[:3]}_{mkt[:5]}_p{n_p}_wr{mwr}",
                "label": "Challenge x3",
                "components": [{
...
viable = [r for r in results if r["cap"] >= 270 and r["compl"] >= 50]
viable.sort(key=lambda r: -r["prob_3"])

print(f"\n[X3] {len(viable)} viables (cap ≥270€ ET compl ≥50%)")
print(f"\n=== TOP 20 par PROB_3 ===")
for r in viable[:20]:
    flag = " 🏆" if r["prob_3"] >= 95 else ""
    print(f"  P3 {r['prob_3']:>5.1f}% P5 {r['prob_5']:>5.1f}% P10 {r['prob_10']:>5.1f}% | compl {r['compl']:>4.0f}% cap {r['cap']:>4.0f}€ #{r['n_total']:>2d} | {r['id'][:55]}{flag}")

with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/new_v17_x3.json","w") as f:
