#!/usr/bin/env python3
"""Combos legs=2-3 en mode INTERDAY (1 palier/jour) — angle peu exploré."""
import sys, os, json
...
for sports, sname in [(["football","ice-hockey"],"FH"),
                       ...
                       (["football"],"F")]:
    for cmin, cmax in [(1.20,1.35), (1.25,1.40), (1.30,1.45), (1.20,1.40)]:
        for mwr in [0.55, 0.60, 0.65, 0.70, 0.75]:
            for legs in [2, 3]:
                for n_p in [3, 5, 7]:
                    for sort in ["wr", "ev"]:
                        CANDS.append({
                            "id": f"CI_{sname}_{cmin}-{cmax}_wr{int(mwr*100)}_l{legs}_p{n_p}_{sort}",
...
for i, s in enumerate(CANDS):
    ...
    try:
        ...
        if r["n_cycles_total"] >= 8 and r["final_pnl"] > 1500:
            results.append({
                "id": s["id"],
...
print(f"\n[ci] {len(results)} viable")
results.sort(key=lambda r: -r["pnl"])
print(f"\n=== TOP 15 par PnL ===")
for r in results[:15]:
    score = r["comp"] * r["cap"] * r["n_tot"]/100
    print(f"  score={score:>5.0f} {r['comp']*100:>5.1f}% {r['n_comp']:>3}/{r['n_tot']:<3} cap{r['cap']:>5.0f} {r['pnl']:>+6.0f}  {r['id']}")
...
with open("/tmp/find_combo_interday.json","w") as f: json.dump(results,f,indent=2)
print("\nSaved")
