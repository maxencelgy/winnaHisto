#!/usr/bin/env python3
"""Sweep 5-sports — Foot+Hockey+Basket+Tennis+Baseball multi-comp."""
import sys, os, json
...
for foot_mc in [3, 5]:
    for hockey_mc in [3, 5]:
        for basket_mc in [1, 2]:
            for tennis_mc in [1, 2]:
                for mlb_mc in [1, 2]:
                    for pct in [0.05, 0.07]:
                        CANDS.append({
                            "id": f"5S_F{foot_mc}H{hockey_mc}B{basket_mc}T{tennis_mc}M{mlb_mc}_pct{int(pct*100)}",
                            "components": [
                                ...
                                {"sport": "baseball", "market": "1x2",
                                 "cote_min": 1.30, "cote_max": 1.60,
                                 "sort_by": "wr", "max_legs": 1, "max_combos": mlb_mc,
...
# B. Sizing extra-aggro sur F3H3 (DD bas)
for sizing_pct in [0.10, 0.12, 0.15, 0.18, 0.20]:
    CANDS.append({
        "id": f"AGRO_F3H3_pct{int(sizing_pct*100)}",
        "components": [
            {"sport": "football", "market": "btts,over_1_5,over_2_5",
             "cote_min": 1.20, "cote_max": 1.40,
...
print(f"[5-sports + aggro F3H3] {len(CANDS)} configs")
...
print(f"\n[5-sports] {len(viable)} viables (ratio ≥10, BR ≥10)")
...
with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/wfr_5sports.json","w") as f:
