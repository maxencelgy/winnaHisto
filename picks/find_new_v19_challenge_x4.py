...
CANDS = []
# 50→200 = ×4
# 3p × 1.587, 4p × 1.414, 5p × 1.319, 6p × 1.260, 7p × 1.219
configs = [
    (3, 1.55, 1.65, "3p_160"),
    (4, 1.38, 1.46, "4p_142"),
    (4, 1.40, 1.45, "4p_142_strict"),
    (5, 1.28, 1.35, "5p_132"),
    (6, 1.23, 1.30, "6p_126"),
    (7, 1.19, 1.26, "7p_122"),
]
for n_p, cmin, cmax, tag in configs:
    for sport, mkt in [("football", "over_1_5"), ("football", "over_1_5,over_2_5"),
        ...
        for mwr in [0.65, 0.70, 0.75, 0.80]:
            CANDS.append({
                "id": f"X4_{tag}_{sport[:3]}_{mkt[:5]}_p{n_p}_wr{mwr}",
                "label": "Challenge x4 BR50",
                "components": [{
...
viable = [r for r in results if r["cap"] >= 180 and r["compl"] >= 40]
viable.sort(key=lambda r: -r["prob_5"])

print(f"\n[X4] {len(viable)} viables (cap ≥180€ ET compl ≥40%)")
print(f"\n=== TOP 20 par PROB_5 ===")
for r in viable[:20]:
...
with open("/Users/maxenceleguay/Sites/winnaHisto/datasets/new_v19_x4.json","w") as f:
    json.dump({"viable": viable[:30]}, f, indent=2)
print("\nSaved.")
